...
import com.roofapp.ui.utils.messages.Message;
...
//	private CurrentUser currentUser;
	...
	public void delete(CrudOperationListener<T> onSuccess) {
		...
		confirmIfNecessaryAndExecute(true, CONFIRM_DELETE, () -> {
			if (executeOperation(() -> crudService.delete(null/*currentUser.getUser()*/,
				state.getEntity()))) {
	...
	private void saveEntity() {
		state.updateEntity(
			crudService.save(/*currentUser.getUser()*/new User(), state.getEntity()),
			isNew());
	}
	...
	public T createNew() {
		state.updateEntity(crudService.createNew(/*currentUser.getUser()*/null), true);
